    ...
    describe('Approve Stage tests', function() {
        it('should transfer balance to beneficiary', async function() {
      	...
      	it('should allow arbiter to approve', async function() {
            const approve = await contract.approve({from: _arbiter});
      		let balance = web3.eth.getBalance(_beneficiary);
            assert(balance > 0);
        });
      
      	it('should not allow anyone but the arbiter to approve', async function() {
        	await expectThrow(contract.approve({from: _beneficiary}));
        });  
    });
